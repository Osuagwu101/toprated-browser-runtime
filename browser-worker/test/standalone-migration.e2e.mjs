...
assert.equal(shell.headers.has('x-frame-options'), false);
const shellCsp = shell.headers.get('content-security-policy') || '';
assert.ok(shellCsp.includes("frame-ancestors 'self' https://topratedseotools.com https://www.topratedseotools.com"));
assert.equal(shellCsp.includes("frame-ancestors 'none'"), false);
